...
def response_to_commands_high(token_bot, add_message):
    token_bot.send_message(add_message.from_user.id, 'Автомобили отсортированы от максимальной до минимальной цены')
    data_auto = open_file_json.open_file_json_auto()
    sort_data_auto = sorted(data_auto, key=lambda d: d['PRICE'], reverse=True)
